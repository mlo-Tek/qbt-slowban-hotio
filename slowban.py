...
def login() -> None:
    ...
    if result and result not in ("ok.", "ok") and not has_session_cookie:
        raise RuntimeError(f"Login failed: unexpected response HTTP {response.status_code}: {response.text}")
    probe = session.get(f"{QBT_URL}/api/v2/app/version", timeout=15)
    if not (200 <= probe.status_code < 300):
        raise RuntimeError(f"Login verification failed: HTTP {probe.status_code}: {probe.text}")
    log(f"Logged into qBittorrent at {QBT_URL} (HTTP {response.status_code}, session_verified=yes)", "INFO")
...
def is_peer_scan_candidate(torrent: Dict[str, Any]) -> bool:
    """Return True only when a torrent can currently have useful download peers.

    The old implementation queried torrentPeers for every torrent. On libraries with
    ...
    if state in inactive_or_upload_states:
        return False

    # Conservative fallback for unknown/future states: only scan incomplete torrents
    # that qBittorrent reports as actually connected/active.
    amount_left = int(torrent.get("amount_left", 0) or 0)
    dlspeed = int(torrent.get("dlspeed", 0) or 0)
    num_seeds = int(torrent.get("num_seeds", 0) or 0)
    num_leechs = int(torrent.get("num_leechs", 0) or 0)
    return amount_left > 0 and (dlspeed > 0 or num_seeds > 0 or num_leechs > 0)


def get_peers(torrent_hash: str) -> Dict[str, Any] | None:
    response = session.get(
        f"{QBT_URL}/api/v2/sync/torrentPeers",
        params={"hash": torrent_hash, "rid": 0},
        timeout=30,
    )
    # A torrent can disappear/change state between torrents/info and torrentPeers.
    # qBittorrent answers 404 in that race; it is not an authentication failure.
...
def ban_peer(peer_address: str, torrent_name: str, state: Dict[str, Any]) -> None:
    if SLOWBAN_DRY_RUN:
        ...
        return
    response = session.post(f"{QBT_URL}/api/v2/transfer/banPeers", data={"peers": peer_address}, timeout=15)
    response.raise_for_status()
    increment_ban_counters(state)
...
def set_preferences(payload: Dict[str, Any]) -> None:
    response = session.post(f"{QBT_URL}/api/v2/app/setPreferences", data={"json": json.dumps(payload)}, timeout=30)
    response.raise_for_status()
...
def cron_matches(dt: datetime, expr: str) -> bool:
    ...
    return dt.minute in minutes and dt.hour in hours and dt.day in dom and dt.month in months and cron_dow in dow
...
def clear_bans_preserve_permanent(state: Dict[str, Any]) -> None:
    ...
    log(f"Clearing ban list. Existing entries={len(existing)}, unbanned={len(unbanned)}, permanent entries={len(permanent)}", "UNBAN")
...
def maybe_emit_summary(state: Dict[str, Any], torrent_count: int, active_peer_count: int) -> None:
    now_ts = time.time()
    ...
    while True:
        ...
        try:
            ...
            for torrent in torrents:
                ...
                if not torrent_hash or not is_peer_scan_candidate(torrent):
                    continue
                ...
                for peer_address, peer_data in peers.items():
                    ...
                    if should_track_peer(peer_data):
                        ...
                        state[state_key] = {
                            ...
                            "torrent_hash": torrent_hash, "torrent_name": torrent_name,
                            "peer_address": peer_address, "up_speed": up_speed,
                            "client": client, "country": country,
                            "connection": connection, "warned": warned,
                        }
                        if slow_for >= SLOWBAN_WARN_TIME and not warned:
                            log(f"Peer '{peer_address}' on torrent '{torrent_name}' has stayed below {SLOWBAN_MIN_SPEED}B/s for {slow_for}s (current {up_speed}B/s, client='{client}', country='{country}', connection='{connection}'). Ban in {warn_remaining}s if still below threshold.", "WARN")
                            state[state_key]["warned"] = True
                        if slow_for >= SLOWBAN_THRESHOLD_TIME:
                            log(f"Threshold reached for peer '{peer_address}' on torrent '{torrent_name}' after {slow_for}s below {SLOWBAN_MIN_SPEED}B/s (current {up_speed}B/s).", "WARN")
                        ...
                        if previous.get("warned", False):
                            log(f"Peer '{peer_address}' on torrent '{torrent_name}' recovered before ban after {slow_for}s below threshold.", "INFO")
                        state.pop(state_key, None)

            stale_keys = [key for key in list(state.keys()) if key not in STATE_META_KEYS and key not in seen_state_keys]
            ...
            # Only authentication/session failures justify a login. A 404 from a
            # torrent-specific endpoint is a normal race and is handled in get_peers.
